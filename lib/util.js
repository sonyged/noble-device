...
Util.inherits = function(constructor, superConstructor) {
  ...
  if (superConstructor === NobleDevice) {
    ...
    constructor.onScanStop = function() {
      constructor.emitter.emit('scanStop');
    };

    constructor.onError = function(err) {
      constructor.emitter.emit('error', err);
    };
    ...
    constructor.discoverAll = function(callback) {
      ...
      if (constructor.emitter.listeners('discover').length === 1) {
        ...
        noble.on('scanStop', constructor.onScanStop);
        noble.on('error', constructor.onError);
    ...
    constructor.stopDiscoverAll = function(discoverCallback) {
      ...
      if (constructor.emitter.listeners('discover').length === 0) {
        ...
        noble.removeListener('scanStop', constructor.onScanStop);
        noble.removeListener('error', constructor.onError);
